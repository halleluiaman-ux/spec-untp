...
- [Conformity Scheme owners](SchemeOwners.md) will typically implement the CVC specification so that their conformity criteria or digtially referenceable. They (or their accredited auditors) will also implement the DCC specification so that they can provide their customers with verifiable sustainability credentials about their products and facilities.
...
- [CVC](../specification/ConformityVocabularyCatalog.md) - Conformity Vocabulary Catalog

## Registration Process

To register your current or planned implementation please raise a [new issue](https://opensource.unicc.org/un/unece/uncefact/spec-untp/-/issues) and select the "Implementation register template".
...
Registered implementers are requested to provide 6-monthly reporting of anonymised volumetric data as described on the [Impact Assessment Framework](../business-case/ImpactAssessmentFramework.md) so that UN/CEFACT can track ongoing impact of the UNTP. UN/CEFACT will publish anonymised performance reporting that implementers can use to benchmark their own performance against industry and geographical averages.
