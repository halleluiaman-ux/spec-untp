...
1. Signed in to GitLab. You can sign up [here](https://opensource.unicc.org/users/sign_up) if you haven't done it yet.
...
If you only need to make some very simple changes to an existing page (eg fix some typos or modify some content) then the simplest solution is just to use the online editor. On the bottom left of any [UNTP website](https://untp.unece.org/docs/about) page there is an `Edit this page` link.
...
6. Type `yarn start`. This will launch the website and open it in a browser windo on your local machine at `http://localhost:3000`. Whenever you make changes to UNTP Markdown files, you'll see the change on your local website.
