...
async function validateCredentialsSchemas(jsonSchemas) {
  ...
  const results = schemaAndInstancePairs.map(({schema, instance}) => {
    core.info(
      `Validating sample credential "${instance.fileName}" against schema "${schema.fileName}"`
    );
    const validate = ajv.compile(schema.json);
    const isValid = validate(instance.json);

    // Check if all errors are additionalProperties
    const onlyAdditionalPropertiesErrors = validate?.errors?.every(
      error => error.keyword === 'additionalProperties'
    );
    const combinedResult = isValid || onlyAdditionalPropertiesErrors;

    core.info(
      `Sample credential "${instance.fileName}" validation result: ${
        combinedResult ? 'passed' : 'failed'
      }.`
    );
    return {
  ...
  core.info(
    `All sample credentials validation result: ${
  ...
  if (!finalResult) {
    core.setFailed(
      `Sample credentials validation failed: ${JSON.stringify(results)}`
    );
  }
  ...
  try {
    ...
    const pairPromises = schemaFileNames.map(async schemaFileName => {
      ...
      if (!schemaJson || !instanceJson) {
        core.setFailed(
          `Failed to fetch schema "${schemaFileName}" or instance "${instanceFileName}".`
        );
        return null;
      }

      core.info(
        `Fetched schema "${schemaFileName}" and instance "${instanceFileName}".`
      );

      return {
        schema: {
          fileName: schemaFileName,
          url: schemas[schemaFileName],
          json: schemaJson,
        },
        instance: {
...
function splitSchemasAndInstances(jsonSchemas) {
  ...
  for (const schema of jsonSchemas) {
    ...
    if (
      schema.fileName.includes(jsonSchemaSuffix) &&
      !schema.fileName.includes('_instance')
    ) {
...
module.exports = {
  validateCredentialsSchemas,
  pairSchemasAndInstances,
  splitSchemasAndInstances,
};
